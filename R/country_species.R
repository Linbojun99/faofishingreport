...
#' @examples
#' \dontrun{
#' country_species(FAO34, country_name = "Morocco",c(1, 10), plot=TRUE,table=TRUE) #area_data=Fao34
#' }
country_species <- function(area_data, country_name, rank_range = c(1, 10),plot=TRUE,table=TRUE) {
  ...
  if(plot){
    ...
    eoffice::topptx(figure=p1, filename = filename, width=12, height=10)
  ...
  if(plot)
  {
    ...
    filename <- paste0("figures/", gsub("[[:punct:][:space:]]", "_", country_name), "_species_ranked.pptx")
    eoffice::topptx(figure=p2, filename = filename, width=12, height=10)
    ...
    p3<-ggplot2::ggplot() +
      ...
      ggplot2::geom_text(data =country_species_ranked_total %>% dplyr::filter(ASFIS.species..Name. %in% country_species_ranked$ASFIS.species..Name.) %>% dplyr::ungroup() %>%
                         ...
                         hjust = 1, vjust = -3, size = 4.5, color = "black") +
      ggplot2::theme_bw() +
      ggsci::scale_color_npg()+
      ggplot2::labs( x = "Year", y = "Million Tonnes") +
      ggplot2::scale_x_continuous(breaks = seq(1950,2021,5),expand = c(0,0))+
      ...
      ggplot2::guides(color = ggplot2::guide_legend(nrow = 2))
    filename <- paste0("figures/", gsub("[[:punct:][:space:]]", "_", country_name), "_species_ranked.pptx")
    eoffice::topptx(figure=p3, filename = filename, width=12, height=10)
  ...
  if(plot){
    ...
    filename <- paste0("figures/", gsub("[[:punct:][:space:]]", "_", country_name), "_species_ranked.pptx")
    eoffice::topptx(figure=p4, filename = filename, width=12, height=10)
  }
  ...
  # 根据排名范围筛选数据
  decade_avg_production_species_ranked <- decade_avg_production_country %>%
  ...
  recent_production_species_ranked <- recent_production_country %>%
    dplyr::inner_join(country_species_ranked_total, by = "ASFIS.species..Name.") %>%
    dplyr::select(-species_production,-percentage_country)  # 删除不需要的列

  percentage_of_total_species_ranked <- percentage_of_total_country %>%
    dplyr::inner_join(country_species_ranked_total, by = "ASFIS.species..Name.") %>%
    dplyr::select(-species_production,-percentage_country)  # 删除不需要的列
